...
-- Update all entries with null or 'Other' categories
UPDATE spending_entries
SET category = recategorize_entry(description)
WHERE category IS NULL OR category = 'Other' OR category = '';

-- Also update any entries that might have been miscategorized
UPDATE spending_entries
SET category = recategorize_entry(description)
WHERE category NOT IN ('Food & Dining', 'Transportation', 'Shopping', 'Entertainment', 'Health', 'Bills & Utilities', 'Education', 'Personal Care', 'Other');
